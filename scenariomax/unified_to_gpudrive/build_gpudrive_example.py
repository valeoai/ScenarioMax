import numpy as np
import trimesh
from scenariomax.unified_to_gpudrive.data.map_element_ids import MapElementIds, TYPE_TO_MAP_ID, TYPE_TO_MAP_FEATURE_NAME, FILTERED_TYPES
...
def _convert_map_features(scenario_net_map_features):
    ...
    edge_segments = []
    index = 0
    for map_feature_id in scenario_net_map_features.keys():
        ...
        original_geometry = np.expand_dims(original_geometry, 0) if len(original_geometry.shape) == 1 else original_geometry 
        ...
        if "ROAD_EDGE" in feature_type:
            edge_vertices = [
                [r["x"], r["y"], r["z"]] for r in new_feature["geometry"]
            ]
            edge_segments += [
...
def build_gpudrive_example(name, scenario_net_scene, debug=False):
    ...
    return _convert_numpy(scenario_dict)