...
from scenariomax.unified_to_tfrecord.constants import DEFAULT_NUM_ROADMAPS, DIST_INTERPOLATION
from scenariomax.unified_to_tfrecord.converter.datatypes import RoadGraphSamples
...
def get_scenario_map_points(scenario: dict[str, Any], debug: bool = False) -> tuple[RoadGraphSamples, int, bool]:
    ...
    for mp_block in map_features_keys:
        ...
        if feature_type == "STOP_SIGN":
            points_to_add = np.array(feature["position"])
        else:
            points_to_add = np.array(feature[key])
