...
from scenariomax.unified_to_tfrecord.constants import NUM_PATHS, NUM_POINTS_PER_PATH
from scenariomax.unified_to_tfrecord.converter.datatypes import PathSamples
...
def get_scenario_paths(scenario, sdc_trajectory, sdc_heading, roadgraph_samples, debug=False):
    ...
    # Calculate distance to the ego vehicle
    distances_to_sdc = np.linalg.norm(lanes_xyz - sdc_trajectory[0], axis=1)

    # Find lanes that are close to the SDC
    close_lanes_mask = distances_to_sdc < LANE_WIDTH

    if not np.any(close_lanes_mask):
        # If no lanes are close enough, try with a larger threshold
        close_lanes_mask = distances_to_sdc < (LANE_WIDTH * 2)
        if not np.any(close_lanes_mask):
    ...
    for lane_id, idx in unique_indices.items():
        ...
        sdc_vector = np.array([np.cos(sdc_heading[0]), np.sin(sdc_heading[0])])

        # Calculate alignment score (how well the lane aligns with SDC direction)
        alignment_score = np.dot(lane_vector, sdc_vector)

        # Only consider lanes that generally align with the SDC direction
        if alignment_score > YAW_THRESHOLD:
            # Distance score (closer is better)
            distance_score = 1.0 / (1.0 + distances_to_sdc[idx])
    ...
    xyz, valid, ids, arc_length = _gather_all_paths(paths, sdc_trajectory[0], roadgraph_samples)

    if debug:
        for i in range(len(xyz)):
            mask = valid[i] == 1
            ax.plot(xyz[i, mask, 0], xyz[i, mask, 1], ".", color="black", alpha=0.2)

    ego_lane, idx_closest_paths = _sort_paths_and_get_ego_lane(sdc_trajectory, xyz, valid)
    len_ego_path = len(ego_lane)

    # if debug:
    #     ax.plot(ego_lane[:, 0], ego_lane[:, 1], "b", linestyle="dashed")
    ...
    for i, idx in enumerate(idx_closest_paths):
        ...
        # Check if ego_path is contained in the path
        if np.all(np.equal(path_samples.xyz[i, :len_ego_path], ego_lane)):
    ...
    #         if np.mean(path_samples.valid[i]) == 0:
    #             continue

    #         mask = path_samples.valid[i] == 1

    #         if path_samples.on_route[i] == 1:
    #             ax.plot(
    #                 path_samples.xyz[i, mask, 0],
    #                 path_samples.xyz[i, mask, 1],
    #                 "skyblue",
    #                 linestyle="dashed",
    #                 alpha=0.4,
    #             )
