...
def postprocess_gpudrive(
    ...
    **kwargs,
    ...
    """
    Process scenarios and write them as individual JSON files.
...
def merge_dataset_workers(dataset_dir: str, dataset_name: str) -> None:
    """
    Merge JSON files from worker subdirectories into the dataset directory.
    ...
    # dataset_dir already IS the output directory (e.g., output/json/waymo)
    dirs_to_remove = set()
...
def merge_multiple_datasets(output_dir: str, merged_filename: str = None) -> None:
    """
    Merge multiple dataset directories into a single output directory.

    For JSON format, each scenario is already a separate JSON file, so we just
    ...
    Args:
        ...
        merged_filename: Unused parameter (kept for API compatibility)
    """
