...
def single_worker_preprocess(x: list[Any], worker_index: int) -> list[Any]:
    """
...
def write_to_directory_single_worker(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.error(f"Worker {worker_index} encountered an error: {str(e)}")
        return False
    finally:
        pbar.close()
...
def merge_tfrecord_files(output_dir: str, merged_filename: str) -> None:
    ...
    try:
        ...
        with tf.io.TFRecordWriter(merged_file_path) as writer:
            for tfrecord_file in tqdm(tfrecord_files, desc="Merging TFRecord files"):
                try:
                    ...
                    logger.error(f"Error processing file {tfrecord_file}: {str(e)}")
        ...
        logger.error(f"Error during TFRecord merging: {str(e)}")
        raise
...
def merge_json_files(output_dir: str) -> None:
    ...
    try:
        ...
        logger.error(f"Error during JSON file merging: {str(e)}")
        raise
...
def shuffle_tfrecord_file(tfrecord_file: str, buffer_size: int = 10000) -> None:
    ...
    try:
        ...
        logger.error(f"Error during shuffling: {str(e)}")
