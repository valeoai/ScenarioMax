...
class ScenarioType:
    ...
    def is_white_line(cls, line):
        return ScenarioType.is_road_line(line) and not ScenarioType.is_yellow_line(line)
    ...
    def is_stop_sign(cls, type):
        return type == ScenarioType.STOP_SIGN

    @classmethod
    def is_speed_bump(cls, type):
        return type == ScenarioType.SPEED_BUMP

    @classmethod
    def is_driveway(cls, type):
        return type == ScenarioType.DRIVEWAY

    @classmethod
    def is_crosswalk(cls, type):
        return type == ScenarioType.CROSSWALK
    ...
    def simplify_light_status(cls, status: str):
        ...
        if status in [cls.LANE_STATE_UNKNOWN, cls.LANE_STATE_FLASHING_STOP, cls.LIGHT_UNKNOWN, None]:
            ...
            logger.warning(f"TrafficLightStatus: {status} is not ScenarioType")
            return cls.LIGHT_UNKNOWN
    ...
    def __init__(self, type=None):
        ...
        self.scenarionet_type = ScenarioType.UNSET
        if type is not None:
            self.set_scenarionet_type(type)

    def set_scenarionet_type(self, type):
        if type in ScenarioType.__dict__.values():
            # Do something if type matches one of the class variables
            self.scenarionet_type = type
        else:
            raise ValueError(f"'{type}' is not a valid ScenarioType.")
