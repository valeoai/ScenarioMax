...
from scenariomax.raw_to_unified import description
from scenariomax.raw_to_unified import type as scenario_type
from scenariomax.raw_to_unified.datasets.nuscenes import types as nuscenes_types
from scenariomax.raw_to_unified.datasets.nuscenes import utils as nuscenes_utils
...
def convert_nuscenes_scenario(
    ...
    if prediction:
        ...
        frames_scene_info = nuscenes_utils.extract_frames_scene_info(token, nuscenes)

    scenario_log_interval = 0.1
    frames, scene_info = frames_scene_info
    result = description.ScenarioDescription()
    result[description.ID] = scene_info["name"]
    result[description.VERSION] = "nuscenes" + version
    result[description.LENGTH] = (len(frames) - 1) * 5 + 1
    result[description.METADATA] = {}
    result[description.METADATA]["dataset"] = "nuscenes"
    result[description.METADATA]["map"] = nuscenes.get("log", scene_info["log_token"])["location"]
    result[description.METADATA]["date"] = nuscenes.get("log", scene_info["log_token"])["date_captured"]
    # result[description.METADATA]["dscenario_token"] = scene_token
    result[description.METADATA][description.ID] = scene_info["name"]
    result[description.METADATA]["scenario_id"] = scene_info["name"]
    result[description.METADATA]["sample_rate"] = scenario_log_interval
    result[description.METADATA][description.TIMESTEP] = np.arange(0.0, (len(frames) - 1) * 0.5 + 0.1, 0.1)
    # interpolating to 0.1s interval
    result[description.TRACKS] = get_tracks_from_frames(nuscenes, scene_info, frames, num_to_interpolate=5)
    result[description.METADATA][description.SDC_ID] = "ego"

    # No traffic light in nuscenes at this stage
    result[description.DYNAMIC_MAP_STATES] = {}
    if prediction:
        track_to_predict = result[description.TRACKS][instance_token]
        result[description.METADATA]["tracks_to_predict"] = {
            instance_token: {
                "track_index": list(result[description.TRACKS].keys()).index(instance_token),
    ...
    # map
    map_center = np.array(result[description.TRACKS]["ego"]["state"]["position"][0])
    result[description.MAP_FEATURES] = get_map_features(
        scene_info,
        nuscenes,
        map_center,
        map_radius,
...
def get_map_features(scene_info, nuscenes: NuScenes, map_center, radius=500, points_distance=1, only_lane=False):
    ...
    if not only_lane:
        ...
        for idx, boundary in enumerate(boundaries[0]):
            ...
            id = f"boundary_{idx}"
            ret[id] = {description.TYPE: scenario_type.BOUNDARY_LINE, description.POLYLINE: block_points[mask][::-1]}
        ...
        for id in map_objs["lane_divider"]:
            ...
            ret[nuscenes_utils.convert_id_to_int(id)] = {
                description.TYPE: scenario_type.LINE_BROKEN_SINGLE_WHITE,
                description.POLYLINE: line[mask],
            }
        ...
        for id in map_objs["road_divider"]:
            ...
            ret[nuscenes_utils.convert_id_to_int(id)] = {
                description.TYPE: scenario_type.LINE_SOLID_SINGLE_YELLOW,
                description.POLYLINE: line[mask],
            }
        ...
        for id in map_objs["ped_crossing"]:
            ...
            ret[nuscenes_utils.convert_id_to_int(id)] = {
                description.TYPE: scenario_type.CROSSWALK,
                description.POLYGON: boundary_polygon,
            }
        ...
        for id in map_objs["walkway"]:
            ...
            ret[nuscenes_utils.convert_id_to_int(id)] = {
                description.TYPE: scenario_type.BOUNDARY_SIDEWALK,
                description.POLYGON: boundary_polygon,
            }
    ...
    for id in map_objs["lane"]:
        ...
        ret[nuscenes_utils.convert_id_to_int(id)] = {
            description.TYPE: scenario_type.LANE_SURFACE_STREET,
            description.POLYLINE: np.asarray(
                discretize_lane(map_api.arcline_path_3[id], resolution_meters=points_distance),
            ),
            description.POLYGON: boundary_polygon[mask],
            description.ENTRY: [nuscenes_utils.convert_id_to_int(_id) for _id in map_api.get_incoming_lane_ids(id)],
            description.EXIT: [nuscenes_utils.convert_id_to_int(_id) for _id in map_api.get_outgoing_lane_ids(id)],
            description.LEFT_NEIGHBORS: [],
            description.RIGHT_NEIGHBORS: [],
        }
    ...
    for id in map_objs["lane_connector"]:
        ...
        ret[nuscenes_utils.convert_id_to_int(id)] = {
            description.TYPE: scenario_type.LANE_SURFACE_UNSTRUCTURE,
            description.POLYLINE: np.asarray(
                discretize_lane(map_api.arcline_path_3[id], resolution_meters=points_distance),
            ),
            # description.POLYGON: boundary_polygon,
...
def get_tracks_from_frames(nuscenes: NuScenes, scene_info, frames, num_to_interpolate=5):
    ...
    tracks = {
        k: {
            ...
            "metadata": {"track_length": episode_len, "type": scenario_type.UNSET, "object_id": k, "original_id": k},
        }
    ...
    for frame_idx in range(episode_len):
        ...
        for id, state in frames[frame_idx].items():
            # Fill type
            md_type, meta_type = nuscenes_types.get_scenarionet_type(state["type"])
            tracks[id]["type"] = md_type
            tracks[id][description.METADATA]["type"] = meta_type
            if md_type is None or md_type == scenario_type.UNSET:
    ...
    for track in tracks_to_remove:
        ...
        obj_type = track_data[description.METADATA]["type"]
        logger.warning(f"Cannot map type: {obj_type} to any Type")
    ...
    for (
        ...
        # position
        interpolate_tracks[id]["state"]["position"] = nuscenes_utils.interpolate(
        ...
        # velocity
        interpolate_tracks[id]["state"]["velocity"] = nuscenes_utils.interpolate(
        ...
        if max_vel > 30:
            logger.warning(f"Too large speed for {id}: {max_vel}")

        # heading
        # then update position
        new_heading = nuscenes_utils.interpolate_heading(track["state"]["heading"], track["state"]["valid"], new_valid)
        ...
        for k, v in track["state"].items():
            if k in ["valid", "heading", "position", "velocity"]:
                ...
                interpolate_tracks[id]["state"][k] = nuscenes_utils.interpolate(v, track["state"]["valid"], new_valid)
        # if id == "ego":
